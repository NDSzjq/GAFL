...
GAN增强差分隐私联邦学习 - 完整实现
基于对抗学习方案，包含两个阶段：
- 阶段0（第1-30轮）：标准FedAvg训练
- 阶段1（第31-500轮）：GAN增强训练
数据集: CIFAR-10 (32x32x3 RGB图像)
"""
...
ACCURACY_THRESHOLD_FOR_GAN = 28.0  # 准确率阈值：超过28%后才启用生成器数据训练
...
def partition_data_noniid(train_dataset, num_clients=100, num_classes_per_client=2):
    ...
    Non-IID数据划分
    每个客户端只包含指定数量的不同类别
    CIFAR-10: 每个类别5000个样本，每个客户端每类250个样本
    """
...
def partition_data_dirichlet(train_dataset, num_clients=10, alpha=0.1, min_samples=10, seed=42):
    ...
    for client_id in range(num_clients):
        ...
        print(f"  客户端 {client_id}: {num_samples} 样本, {len(classes)} 类别, [{class_info}]")
    ...
    print(f"覆盖类别数: {len(all_classes)}/10")
...
class ClientWithGAN:
    ...
    def train_phase1(self, global_model_params, test_accuracy_history, model, generator, local_epochs=None, use_synthetic_data=True):
        ...
        参数:
            ...
            model: 本地模型
            generator: 生成器模型
            use_synthetic_data: 是否使用生成数据训练（默认True）
        """
        model.load_state_dict(global_model_params)
        ...
        # 步骤1: 训练生成器（热启动）- 无论是否使用生成数据，都训练生成器
        # 检查生成器参数是否匹配当前架构
        ...
        # 只有当use_synthetic_data=True时才生成和使用合成数据
        if use_synthetic_data:
            ...
            effective_local_epochs = self.local_epochs if local_epochs is None else int(local_epochs)
            model, avg_loss, client_grad_norm_median, sample_usage_stats = train_with_mixed_data(
                model, self.data_loader, None, None,  # 不使用生成数据
                epochs=effective_local_epochs, batch_size=self.batch_size,
                real_weight=1.0, synthetic_weight=0.0,  # 真实数据权重100%
                device=self.device, clipping_threshold=self.clipping_threshold,
                global_model_params=global_model_params,
                minority_classes=self.minority_classes,
                tau=self.num_samples / 10,
                lambda_val=0.0  # 生成数据权重为0
            )
...
def main():
    """主函数"""
    print("=" * 60)
    print("GAN增强差分隐私联邦学习实验 (CIFAR-10)")
    print("=" * 60)
    ...
    if torch.cuda.is_available():
        ...
        while True:
            try:
                choice = input(f"\n请选择GPU [0-{num_gpus-1}]，直接输入数字: ")
                GPU_ID = int(choice)
                if 0 <= GPU_ID < num_gpus:
                    break
                else:
                    print(f"输入错误，请输入 0 到 {num_gpus-1} 之间的数字")
            except ValueError:
                print("输入错误，请输入数字")
        
        device = torch.device(f'cuda:{GPU_ID}')
        torch.cuda.set_device(GPU_ID)
        print(f"\n已选择: {device} ({torch.cuda.get_device_name(GPU_ID)})")
    else:
        device = torch.device('cpu')
        print(f"\n未检测到GPU，使用CPU")
    
    # 是否加载第10轮FedAvg模型（跳过阶段0训练）
    # 自动检测：如果模型文件存在则加载，否则重新训练
    ...
    LEARNING_RATE = 0.005
    GLOBAL_ROUNDS = 500
    PHASE0_ROUNDS = 30  # 标准FedAvg轮次
    PHASE1_ROUNDS = 470  # GAN增强轮次
    ...
    # NUM_SYNTHETIC_PER_CLASS: 动态计算 = 0.6 × 客户端总数据量 / 10
    ...
    print(f"模型: ResNet18")
    print(f"模型参数量: {sum(p.numel() for p in global_model.parameters()):,}")
    ...
    def get_or_create_client(client_id):
        ...
        client = ClientWithGAN(
            ...
            gen_lr=GEN_LR,
            # num_synthetic_per_class: 使用默认值，动态计算 = 0.6 × 客户端数据量 / 10
            alpha=ALPHA,
            beta=BETA,
    ...
    if LOAD_PHASE0_MODEL and os.path.exists('fedavg_round10_model.pth'):
        ...
        print("已加载 fedavg_round10_model.pth")
        
        # 评估加载的模型
        test_accuracy, test_loss = server.evaluate()
        print(f"加载模型测试准确率: {test_accuracy:.2f}%")
    ...
    # 动态阶段训练循环
    print("\n" + "=" * 60)
    print(f"动态阶段训练开始 (准确率阈值: {ACCURACY_THRESHOLD_FOR_GAN}%)")
    print("=" * 60)
    ...
    for round_idx in range(current_round, GLOBAL_ROUNDS + 1):
        ...
        if best_accuracy >= ACCURACY_THRESHOLD_FOR_GAN:
            ...
            if phase_switch_round is None:
                ...
                print(f"\n{'='*60}")
                print(f"最高准确率达到 {ACCURACY_THRESHOLD_FOR_GAN}%，切换到阶段1（GAN增强训练）")
                print(f"阶段切换轮次: 第{phase_switch_round}轮")
                print(f"{'='*60}\n")
        else:
            current_phase = 0
        
        print(f"\n全局轮次 {round_idx}/{GLOBAL_ROUNDS} (阶段{current_phase})")
        ...
        print(f"  最高准确率: {best_accuracy:.2f}% (第{best_round}轮)")
        
        # 阶段0：每轮结束后训练每个客户端的生成器（预热）
        if current_phase == 0:
    ...
    print(f"最高测试准确率: {best_accuracy:.2f}% (第{best_round}轮)")
    ...
    print(f"训练历史已保存到: gan_dp_training_history.npy")
