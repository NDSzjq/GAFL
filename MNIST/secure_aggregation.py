...
def verify_secure_aggregation():
    ...
    for i, client in enumerate(clients):
        ...
        print(f"  NOTE {i} NOTE: {update.norm().item():.4f}")
    ...
    for i, client in enumerate(clients):
        ...
        # NOTE: (params, num_samples, client_id)
        masked_updates.append((masked_params, 1, i))
        
        print(f"  NOTE {i} NOTE: {masked_params['weight'].norm().item():.4f}")
    ...
    # NOTE
    print("\nNOTE:")
    print(f"  NOTE: {aggregated_params['weight'].norm().item():.6f}")
    print(f"  NOTE: {original_avg.norm().item():.6f}")
    ...
    print(f"  NOTE: {max_diff:.6e}")
    print(f"  NOTE: {mean_diff:.6e}")
    
    # NOTE
    tolerance = 1e-5
    if max_diff < tolerance:
        print(f"\nPASS NOTE!NOTE {tolerance}")
        return True
    else:
        print(f"\nFAIL NOTE!NOTE {tolerance}")
        return False
...
def test_dp_integration():
    ...
    """
    print("\n" + "=" * 60)
    print("NOTE + NOTEDP NOTE")
    print("=" * 60)
    
    # NOTE
    num_clients = 5
    seed = 42
    noise_sigma = 0.1  # DPNOTE
    ...
    for i, client in enumerate(clients):
        ...
        print(f"  NOTE {i}:")
        print(f"    NOTE: {update.norm().item():.4f}")
        print(f"    NOTE: {noisy_update.norm().item():.4f}")
    ...
    print("\nNOTE:")
    print(f"  NOTE: {aggregated_params['weight'].norm().item():.6f}")
    print(f"  NOTE: {expected_avg.norm().item():.6f}")
...
if __name__ == '__main__':
    ...
    print("NOTE")
    print("=" * 60)
    print(f"  NOTE: {'NOTE' if test1_passed else 'NOTE'}")
    print(f"  DPNOTE: {'NOTE' if test2_passed else 'NOTE'}")
